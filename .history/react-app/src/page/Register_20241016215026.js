...
function Account() {
    ...
    const [editedGender, setEditedGender] = useState('Other'); // เพิ่ม state สำหรับ gender
    const [editedAge, setEditedAge] = useState(''); // เพิ่ม state สำหรับ age
    ...
    const getAccount = () => {
        axios.get("http://localhost:4000/account", {
          ...
          setEditedGender(res.data[0]?.gender || 'Other'); // ตั้งค่า gender
          setEditedAge(res.data[0]?.age || ''); // ตั้งค่า age
        }).catch(err => {
          console.log(err);
        });
    };
    ...
    const gender = account[0]?.gender; // ดึง gender
    const age = account[0]?.age; // ดึง age
